...
public class PartDeckMission extends GridPane implements ApplicationMainPart {
	private final Label[] nameLabels;
	private final Label[] timeLabels;

	public PartDeckMission(UnitManager unitManager, Stage primaryStage) {
		int length = 4;

		this.nameLabels = IntStream.range(0, length).mapToObj(index -> {
			Label label = new Label("远征" + (index + 1));
