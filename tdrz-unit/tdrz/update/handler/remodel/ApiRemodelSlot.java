...
public class ApiRemodelSlot extends UnitHandler {
	...
	private final List<WordSlotItem> newItem;
	...
	public ApiRemodelSlot(UnitManager unitManager,long time, Map<String, String> fields, JsonValue api_data) {
		...
		this.newItem = Optional.ofNullable(json.getJsonObject("api_after_slot")).map(WordSlotItem::new).map(FunctionUtils::asList).orElse(Collections.emptyList());
