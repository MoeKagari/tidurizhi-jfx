...
public class ShipDtoTranslator {
	...
	public static int getSuodi(WordShip ship) {
		...
		int suodi = 0;
		for (int i = 0; i < 4; i++) {
			suodi += SlotItemDtoTranslator.getSuodi(ship.getSlots()[i]);
		}
		return suodi;
	}
	...
	public static int getZhikong(WordShip ship) {
		...
		int zhikong = 0;
		for (int i = 0; i < 4; i++) {
			zhikong += SlotItemDtoTranslator.getZhikong(ship.getSlots()[i], ship.getOnSlot()[i]);
		}
		return zhikong;
	}
	...
	public static boolean isInNyukyo(WordShip ship) {
		if (ship == null) return false;
		return Arrays.stream(UnitManager.getUnitManager().ndockUnit.ndockHodlers).map(NdockHodler::getNdock).anyMatch(ndock -> //
	...
	public static boolean isInMission(WordShip ship) {
		if (ship == null) return false;
		return Arrays.stream(UnitManager.getUnitManager().deckUnit.deckHolders).map(DeckHolder::getDeck).anyMatch(deck ->//
	...
	public static int whichDeck(WordShip ship) {
		if (ship != null) {
			for (int i = 0; i < 4; i++) {
				if (DeckDtoTranslator.isShipInDeck(UnitManager.getUnitManager().deckUnit.deckHolders[i].getDeck(), ship.getId())) {
					return i;
				}
			}
		}
